...
export default async function ProfilePage() {
  ...
  return (
    <SiteShell activeNav="profile">
      <section className="fade-rise rounded-[42px] border border-[var(--line)] bg-[rgba(255,255,255,0.78)] px-7 py-8 md:px-10 md:py-10">
        <div className="grid gap-8 lg:grid-cols-[1fr_auto]">
          <div className="flex flex-col gap-6 md:flex-row md:items-center">
            <div className="relative flex h-24 w-24 items-center justify-center rounded-full bg-[var(--midnight)] text-white">
              <ProfileAvatar />
              <span className="absolute bottom-1 right-1 h-4 w-4 rounded-full border-4 border-[var(--surface)] bg-[var(--sage)]" />
            </div>

            <div className="max-w-3xl">
              <h1 className="text-[2.8rem] font-semibold tracking-[-0.06em] text-[var(--midnight)]">{profile.displayName}</h1>
              <p className="mt-3 text-[1.04rem] leading-8 text-[var(--slate)]">
          ...
          <div className="flex flex-wrap gap-3 lg:justify-end">
            <button className="nav-link rounded-full border border-[var(--line-strong)] px-5 py-3 text-base text-[var(--midnight)] hover:bg-white">
              设置
            </button>
      ...
      <section className="fade-rise mt-8 grid gap-4 lg:grid-cols-3">
      ...
      <section className="fade-rise mt-12">
  ...
  return (
    <article className="rounded-[26px] border border-[var(--line-strong)] bg-white px-6 py-5">
      <div className="flex items-center justify-between gap-4">
        ...
        <p className="text-[2.35rem] font-semibold leading-none tracking-[-0.06em] text-[var(--midnight)]">{value}</p>
      </div>
    </article>
  );
}
...
function SectionHeader({ title }: { title: string }) {
  return (
    <div className="flex items-end justify-between gap-4">
      <h2 className="text-[2.5rem] font-semibold tracking-[-0.06em] text-[var(--midnight)]">{title}</h2>
    </div>
  );
}
